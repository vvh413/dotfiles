...
-- }
-- ```
xplr.config.modes.builtin.default.key_bindings.on_key.q = xplr.config.modes.builtin.default.key_bindings.on_key["enter"]
...
require("xpm").setup({
	plugins = {
		...
		"sayanarijit/xclip.xplr",
		{
			...
			setup = function()
				require("dragon").setup({ mode = "default", bin = "dragon-drop" })
			end,
		},
		{
			"Junker/nuke.xplr",
			setup = function()
				require("nuke").setup({
					open = {
						custom = {
							{ mime_regex = "^image/.*", command = "xdg-open {} &" },
							{ mime_regex = "^video/.*", command = "xdg-open {} &" },
						},
					},
				})
			end,
