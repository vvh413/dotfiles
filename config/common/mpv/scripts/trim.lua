...
local function getTrimmingPositionsText()
	local function formatSeconds(seconds)
		...
		if seconds > 3600 then
			formatted = string.format("%d:%s", math.floor(seconds / 3600), formatted)
		end
		return formatted
	end

	return "Trimming: "
		.. tostring(formatSeconds(startPosition, true))
		.. " secs ~ "
...
function writeOut()
	...
	-- Error Handlings
	--
	if startPosition == nil or startPosition == "none" or endPosition == nil or endPosition == "none" then
	...
	local args = {
		...
		strip_metadata .. "-avoid_negative_ts",
		"make_zero",
		"-async",
		"1",
